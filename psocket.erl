...
psocket(Socket, Username, Mailbox) ->
    case recv(Socket, 0) of
        {error, Reason} ->
            fwrite("Se ha producido un error en el cliente ~p: ~p.~n", [Username, Reason]);

        {ok, CMD} ->
            pbalance!{self(), where},
            receive {BestNode} ->
                fwrite(">> Usuario ~p ejecutado comando ~p en ~p.~n", [Username, CMD, BestNode]),
                spawn(BestNode, pcomando, pcomando, [Socket, Username, CMD, self(), Mailbox]),
